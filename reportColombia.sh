...
if [ ! -f "${STORAGEDIR}/${REPORTCO}.csv" ]; then
  echo "${STORAGEDIR}/${REPORTCO}.csv not available"
  exit 2
fi
TMPFILE=$(mktemp)
# Se genera el reporte
cat ${STORAGEDIR}/${REPORTCO}.csv | tail -n +2 |  cut -d ',' -f 5 | sort --ignore-case | uniq -ic > ${STORAGEDIR}/${LASTREPORTCO}
if [ -f ${STORAGEDIR}/${LASTREPORTCO}.last ]; then 
  # El archivo existe se compara con el actual reporte
  diff ${STORAGEDIR}/${LASTREPORTCO}.last ${STORAGEDIR}/${LASTREPORTCO} > ${TMPFILE}
  DIFFLINES=$(wc -l ${TMPFILE} | awk '{print $1}')
  if [ ${DIFFLINES} -eq 0 ]; then
    rm ${STORAGEDIR}/${LASTREPORTCO}
    echo 0
    exit 0
  fi
fi
# El reporte actual sera el ultimo reporte
mv ${STORAGEDIR}/${LASTREPORTCO} ${STORAGEDIR}/${LASTREPORTCO}.last
echo 1
exit 1
